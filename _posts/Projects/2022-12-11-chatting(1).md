...
* chat Front Server : port:8081 - 전반적인 에러처리 및 Kafka 백본망에 연결된다. 또한 웹소켓을 여기서 관리한다.
* chat Backend Server : port:8083, port:8084 - 실제 DB와 연결되어 로직을 처리하는 백엔드 서버이다. consumerGroup:groupId=chat
